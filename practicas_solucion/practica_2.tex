...
\RightLabel{T-App}
\BinaryInfC{$\judgeType{\phi}{(\lambdaAbs{x}{\sigma}{x})~(\lambdaAbs{y}{\Bool}{0})}{\sigma}$}
    \end{scprooftree}
...
\vspace*{5mm}
   ...
   \def\extraVskip{5pt}
    ...
    \UnaryInfC{$\judgeType{\Gamma}{y}{Nat}$}
        ...
        \AxiomC{$\Bool\neq\Nat$}
    \RightLabel{T-Union}    \TrinaryInfC{$\judgeType{\Gamma}{[(\lambdaAbs{x}{\Bool}{y},\lambdaAbs{x}{Nat}{x})]}{\text{Union}(\Nat,\Bool)_\Nat}$}
...
\def\extraVskip{5pt}
           ...
           \AxiomC{Demostrado en (1)}
       \UnaryInfC{$\judgeType{\Gamma,x:\text{det}(\Bool)}{\lambdaIf{y}{continuar(x)}{false}}{\Bool}$}
            \AxiomC{}
        \RightLabel{T-Zero}
