...
Podemos usar \mintinline{javascript}{typeof} para saber el nombre del tipo de la expresión.

\begin{itemize}
\item \texttt{Arrays:} \mintinline{javascript}{[], [1,2,true], new Array()}

\mintinline{javascript}{-[-] , push(-) , pop() , shift() , unshift(_)}
\end{itemize}
...
\paragraph{Herencia}
Todos los objetos tiene una propiedad privada llamada \mintinline{javascript}{[[Prototype]]} cuyo valor es \mintinline{javascript}{null} u otro objeto que es su \textbf{prototipo}. Esta propiedad induce una cadena de prototipado sin ciclos que finaliza con \mintinline{javascript}{null}.

Cuando intentamos acceder a un método inexistente de un objeto, el mismo se busca en la cadena de prototipado del mismo hasta encontrar el primer objeto de la cadena que lo define. Si llegamos a \mintinline{javascript}{null} y el método no fue encontrado, entonces hay un error.
...
Cuando ejecutamos un método de la cadena de prototipado, el método evaluado liga \mintinline{javascript}{this} al objeto llamó al método, es decir, si tenemos:
...
Para crear una copia de un objeto y asignarle a esa copia el objeto original como prototipo usamos \mintinline{javascript}{Object.create}.

Además javascript provee el prototipo \mintinline{javascript}{Object.prototype} que es prototipo de todos los objetos y provee metodos básicos como \mintinline{javascript}{hasOwnProperty()} que indica si el objeto contiene una propiedad no heredada y \mintinline{javascript}{toString()} que devuelve un string que representa al receptor.
...
\paragraph{Constructores} Son funciones que generan objetos. Cuando declaramos un constructor \mintinline{javascript}{C}, javascript crea un objeto llamado \mintinline{javascript}{C.prototype} que se asigna como prototipo de todos los objetos creados con dicha función. Por ejemplo:
