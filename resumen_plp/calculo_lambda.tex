...
Veamos un ejemplo: Queremos saber si el siguiente juicio es correcto:
...
\begin{scprooftree}{1}
    \def\extraVskip{10pt}
    \AxiomC{$x:Bool\in\{x:Bool, y:Bool\}$}
    \RightLabel{(T-Var)}
    \UnaryInfC{$\judgeType{\{x:Bool, y:Bool\}}{x}{Bool}$}
    
    \AxiomC{}
    \RightLabel{(T-True)}
    \UnaryInfC{$\judgeType{\{x:Bool, y:Bool\}}{true}{Bool}$}
    
    \AxiomC{$y:Bool\in\{x:Bool, y:Bool\}$}
    \RightLabel{(T-Var)}
    \UnaryInfC{$\judgeType{\{x:Bool, y:Bool\}}{y}{Bool}$}
    
    \RightLabel{(T-If)}
    \TrinaryInfC{$\judgeType{\{x:Bool, y:Bool\}}{\lambdaIf
            {x}
            {true}
            {y}}{Bool}$}   
    
    \RightLabel{(T-Abs)}
    \UnaryInfC{$\judgeType{\{x:Bool\}}{\lambdaAbs{y}{Bool}{\lambdaIf{x}{true}{y}}}{Bool \to Bool}$}
    
    \RightLabel{(T-Abs)}
    \UnaryInfC{$\judgeType{\phi}{\lambdaAbs{x}{Bool}{\lambdaAbs{y}{Bool}{\lambdaIf{x}{true}{y}}}}{Bool \to Bool \to Bool}$}
    ...
    \RightLabel{(T-App)}
    \BinaryInfC{$\judgeType{\phi}{\lambdaApp{(\lambdaAbs{x}{Bool}{\lambdaAbs{y}{Bool}{\lambdaIf{x}{true}{y}}})}{false}}{Bool \to Bool}$}
\end{scprooftree}
\vspace*{\fill}
\end{landscape}
